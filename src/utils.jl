# Core network creation and analysis functions

function create_delaunay_graph!(system::System)
    # Set the random seed for reproducibility
    Random.seed!(system.random_seed)
    println("Random seed set to: ", system.random_seed)
    ...
    # Extract edges from triangulation with debugging
    edges = Set{Tuple{Int,Int}}()
    for triangle in get_triangles(tri)
        i, j, k = triangle
        println("Triangle indices: $i, $j, $k")
        # Check if any index is negative or out of bounds
        if i <= 0 || j <= 0 || k <= 0 || i > size(X, 1) || j > size(X, 1) || k > size(X, 1)
            println("⚠️ Invalid triangle: $i, $j, $k")
            continue
        end
